...
# This will have this script check for a new version of Sinon every
...
download() {
    if [ $DOWNLOAD = true ]; then
        URL=$(curl -s https://api.github.com/repos/TryNotDying/Sinon/releases/latest \
...
run() {
    java -Dnogui=true -jar $(ls -t Sinon* | head -1)
}
